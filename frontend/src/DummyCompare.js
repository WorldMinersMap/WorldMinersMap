    ...
    <Container maxWidth="lg">
      <Box sx={{ mt: 8 }}>
        ...
        {filteredMiners.map((miner) => (
          <Marker 
            key={miner._id} 
            position={[miner.location.coordinates[1], miner.location.coordinates[0]]}
          >
            <Popup>
              <div>
                <h3>{miner.name}</h3>
                <p>Operation Type: {miner.operationType}</p>
                <p>Total Capacity: {miner.totalCapacity}</p>
                <p>Energized Miners: {miner.energizedMiners}</p>
                <p>Region: {miner.region}</p>
                <p>Country: {miner.country}</p>
                {miner.website && <p>Website: <a href={miner.website} target="_blank" rel="noopener noreferrer">{miner.website}</a></p>}
              </div>
            </Popup>
          </Marker>
        ))}
